...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_r611 IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    MODIFY ENTITIES OF zi_rap_travel_r611 IN LOCAL MODE
        ENTITY Travel
          UPDATE
            FIELDS ( TravelStatus )
...
* Comment out the OLD code to validate Agency from DB via SELECT instead use CDS Custom entity ODATA call to external system
*    " Read relevant travel instance data
*    READ ENTITIES OF zi_rap_travel_R611 IN LOCAL MODE
*      ENTITY Travel
...
*        APPEND VALUE #( %tky        = travel-%tky
*                        %state_area = 'VALIDATE_AGENCY'
*                        %msg        = NEW zcm_rap_r611(
*                                          severity = if_abap_behv_message=>severity-error
*                                          textid   = zcm_rap_r611=>agency_unknown
...
*      " Clear state messages that might exist
    ...
    DATA business_data TYPE TABLE OF zrap_R611z_travel_agency_es5.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_r611(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_r611=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_r611(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_r611=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_r611(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_r611=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
 ...
 "   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
   ...
   IF is_update_requested = abap_true.
         ...
         IF has_before_image = abap_true.
           ...
           IF update_granted = abap_false.
             APPEND VALUE #( %tky        = travel-%tky
                             %msg        = NEW zcm_rap_R611( severity = if_abap_behv_message=>severity-error
                                                             textid   = zcm_rap_R611=>unauthorized )
                           ) TO reported-travel.
           ENDIF.
  ...
  METHOD is_create_granted.
     AUTHORITY-CHECK OBJECT 'ZOSTATR611'
      ID 'ZOSTATR611' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATR611'
        ID 'ZOSTATR611' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATR611'
        ID 'ZOSTATR611' DUMMY
